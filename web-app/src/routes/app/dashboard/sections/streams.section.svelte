<script lang="ts">
  import TokenStreamIcon from 'design-system/icons/TokenStreams.svelte';
  ...
  function updateTable() {
    ...
    if (tokenAddress) {
      const byToken = (stream: Stream) =>
        stream.ktrhsConfig.amountPerSecond.tokenAddress.toLowerCase() ===
  ...
  function onRowClick(
    ...
    onClickGoto(
      `/app/${parsedId.senderUserId}/tokens/${parsedId.tokenAddress}/streams/${parsedId.ktrhId}`,
      event.detail.event,
    );
  }
</script>
