<script lang="ts">
  ...
  onMount(async () => {
    try {
      ...
      ktrhsUserId = decodedUserId.ktrhsUserId;
    } catch (e) {
  ...
  $: ktrhsUserId && fetchRequestedAccount(ktrhsUserId);

  
</script>

<svelte:head>
  <title>{(address && $ens[address]?.name) ?? address ?? userId} | Ktrhs</title>
  <meta name="description" content="Ktrhs User Profile" />
</svelte:head>
...
{:else}
  <div class="profile">
    ...
    <Balances userId={ktrhsUserId} />
    <Streams userId={ktrhsUserId} />
