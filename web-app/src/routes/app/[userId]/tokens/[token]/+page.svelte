<script lang="ts">
  ...
  import ArrowUp from 'design-system/icons/ArrowUp.svelte';
  import Plus from 'design-system/icons/Plus.svelte';
  import Minus from 'design-system/icons/Minus.svelte';
  ...
  async function checkUrlUserId() {
    ...
    const connectedToRightUser = checkIsUser(decodedUrlUserId.ktrhsUserId);
...
<svelte:head>
  <title>{token?.info.name ?? 'Unknown Token'} | Ktrhs</title>
  <meta name="description" content="Ktrhs Token Page" />
</svelte:head>
