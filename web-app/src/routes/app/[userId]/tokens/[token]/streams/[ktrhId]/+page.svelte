<script lang="ts">
  ...
  import PauseIcon from 'design-system/icons/Pause.svelte';
  import PlayIcon from 'design-system/icons/Play.svelte';
  import DeleteIcon from 'design-system/icons/Trash.svelte';
  ...
  const { userId, token: tokenAddress, ktrhId } = $page.params;

  let ktrhsUserId: string | undefined;
  ...
  $: {
    $streams;
    if (streamId && ktrhsUserId) {
      stream = streams.getStreamById(streamId);
    }
  }

  $: {
    if (stream && ktrhsUserId && streamId) {
      streamHistory = getStreamHistory(
        streams.getAssetConfig(ktrhsUserId, tokenAddress) ?? unreachable(),
        streamId,
      );
    }
  }
  ...
  $: streamScheduledStart = stream?.ktrhsConfig.startDate;
  ...
  $: streamStartDate =
    stream && streamHistory
      ...
      : undefined;
  $: streamEndDate =
    streamStartDate && stream?.ktrhsConfig.durationSeconds
      ? new Date(streamStartDate.getTime() + stream?.ktrhsConfig.durationSeconds * 1000)
      : undefined;
  ...
  $: {
    if (estimate && stream) {
      if (stream?.paused) {
        ...
        stream?.ktrhsConfig.startDate &&
        stream.ktrhsConfig.startDate.getTime() > new Date().getTime()
      ) {
  ...
  async function getStreamInfo() {
    ...
    try {
      ...
      ktrhsUserId = (await decodeUserId(userId)).ktrhsUserId;
      streamId = makeStreamId(ktrhsUserId, tokenAddress, ktrhId);
    } catch {
      error = 'invalid-id';
      return;
    }
    ...
    try {
      ...
      await streams.fetchAccount(ktrhsUserId);
      stream = streams.getStreamById(streamId);
      assert(stream);
    } catch {
      error = 'not-found';
...
<svelte:head>
  <title>{stream?.name ?? 'Stream'} | Ktrhs</title>
</svelte:head>
  ...
  {:else if streamId && stream}
    <div class="stream-page" in:fly={{ duration: 300, y: 16 }}>
      <div class="hero">
        <div class="title-and-state">
          ...
          <StreamStateBadge
            ...
            durationSeconds={stream.ktrhsConfig.durationSeconds}
            startDate={stream.ktrhsConfig.startDate}
            {tokenAddress}
          />
        </div>
      ...
      <StreamVisual
        ...
        amountPerSecond={stream.ktrhsConfig.amountPerSecond.amount}
        tokenInfo={{
        ...
        <div class="key-value-row">
          <div class="key-value">
            ...
            <span class="value typo-text-mono-bold">
              <FormattedAmount
                decimals={token?.info.decimals ?? unreachable()}
                amount={$balances.accounts[ktrhsUserId ?? unreachable()].tokens[
