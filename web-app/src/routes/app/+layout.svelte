<script lang="ts">
  ...
  async function initializeStores() {
    ...
    if (connected) {
      ...
      if the user is using the WalletConnect Safe App to connect to Ktrhs, instead of using Ktrhs as a Safe App
  ...
  async function warnIfSafe(chainId: number, address: string) {
    ...
    if (isASafe) {
      globalAdvisoryStore.add((resolve) => ({
        ...
        description:
          'Instead of connecting to the Safe with WalletConnect, we recommend running Ktrh as a Safe App directly.',
        emoji: '⚠️',
        button: {
          label: 'Disconnect',
          handler: () => {
