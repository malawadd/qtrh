...
const WEB_3_MODAL_PROVIDER_OPTIONS = {
  walletconnect: {
    ...
    options: {
      rpc: {
        3141: "https://filecoin-hyperspace.chainup.net/rpc/v1"
      },
    },
  },
};
...
const INITIAL_STATE: DisconnectedWalletStoreState = {
  connected: false,
  network: initNetwork,
  provider: new ethers.providers.JsonRpcProvider("https://filecoin-hyperspace.chainup.net/rpc/v1"),
};
...
const walletStore = () => {
  ...
  async function _setConnectedState(
    ...
    state.set({
      connected: true,
      address: accounts[0],
      ktrhsUserId: await (await AddressDriverClient.create(provider, signer)).getUserId(),
      provider,
      signer,
