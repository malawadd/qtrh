...
import seperateKtrhsSetEvents from './methods/separate-ktrhs-set-events';
import buildAssetConfigs from './methods/build-asset-configs';
import { getAddressDriverClient, getSubgraphClient } from '$lib/utils/get-clients';
import mapFilterUndefined from '$lib/utils/map-filter-undefined';
import { reconcileKtrhsSetReceivers } from './methods/reconcile-ktrhs-set-receivers';
...
export const ktrhsConfigSchema = z.object({
  raw: z.string(),
  ktrhId: z.string(),
  ...
  /**
   * If undefined, the block timestamp from the initial setKtrhs event
...
export const streamMetadataSchema = z.object({
  id: z.string(),
  initialKtrhsConfig: ktrhsConfigSchema,
  receiver: userSchema,
  archived: z.boolean(),
    ...
    assetConfigs: mapFilterUndefined(forAccount.assetConfigs, (assetConfig) => ({
      ...
      streams: mapFilterUndefined(assetConfig.streams, (stream) => {
        ...
        return {
          id: stream.id,
          initialKtrhsConfig: {
            ktrhId: stream.ktrhsConfig.ktrhId,
            raw: stream.ktrhsConfig.raw.toString(),
            startTimestamp: Math.floor((stream.ktrhsConfig.startDate?.getTime() || 0) / 1000),
            durationSeconds: stream.ktrhsConfig.durationSeconds || 0,
...
export async function fetchAccount(userId: UserId): Promise<Account> {
  ...
  const ktrhsSetEvents = await subgraphClient.getKtrhsSetEventsByUserId(userId);
  const ktrhsSetEventsWithFullReceivers = reconcileKtrhsSetReceivers(ktrhsSetEvents);
  const ktrhsSetEventsByTokenAddress = seperateKtrhsSetEvents(ktrhsSetEventsWithFullReceivers);

  const assetConfigs = buildAssetConfigs(userId, data, ktrhsSetEventsByTokenAddress);
