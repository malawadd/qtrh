<script lang="ts">
  ...
  import { AddressDriverPresets, constants, Utils } from 'ktrh';
  ...
  const token =
    tokens.getByAddress(stream.ktrhsConfig.amountPerSecond.tokenAddress) ?? unreachable();
  ...
  let newAmountValue: string | undefined =
    ...
    formatUnits(
      stream.ktrhsConfig.amountPerSecond.amount / BigInt(newSelectedMultiplier),
  ...
  $: amountUpdated = newAmountPerSecond !== stream.ktrhsConfig.amountPerSecond.amount;
  $: canUpdate =
    ...
    transact(
      ...
      makeTransactPayload({
        before: async () => {
          ...
          const { ktrhsUserId, address, signer } = $wallet;
          ...
          if (amountUpdated) {
            ...
            const currentStreamReciverIndex = newReceivers.findIndex(
              (r) =>
                Utils.KtrhsReceiverConfiguration.fromUint256(r.config).ktrhId ===
                BigInt(stream.ktrhsConfig.ktrhId),
            );
            newReceivers.splice(currentStreamReciverIndex, 1, {
              userId: stream.receiver.userId,
              config: Utils.KtrhsReceiverConfiguration.toUint256({
                ktrhId: BigInt(stream.ktrhsConfig.ktrhId),
                start: BigInt(stream.ktrhsConfig.startDate?.getTime() ?? 0 / 1000),
                duration: BigInt(stream.ktrhsConfig.durationSeconds ?? 0),
          ...
          if (amountUpdated && nameUpdated) {
            ...
            tx = addressDriverClient.setKtrhs(
              token.info.address,
              currentReceivers,
              newReceivers,
              address,
        ...
        */
          await expect(
            ...
            () =>
              nameUpdated
                ? get(streams).accounts[transactContext.ktrhsUserId].lastIpfsHash ===
                  transactContext.newHash
                : streams.getStreamById(stream.id)?.ktrhsConfig.amountPerSecond.amount ===
                  newAmountPerSecond,
            5000,
            1000,
          );
