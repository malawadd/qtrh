<script lang="ts">
  ...
  $: estimate =
    $balances.accounts[String($wallet.ktrhsUserId) ?? unreachable()].tokens[$context.tokenAddress]
  ...
  function getAssetConfigHistory(ktrhsUserId: string, tokenAddress: string) {
    return (
      get(streams).accounts[ktrhsUserId].assetConfigs.find(
  ...
  function triggerWithdraw() {
    transact(
      ...
      makeTransactPayload({
        before: async () => {
          const { address, ktrhsUserId } = $wallet;
          assert(address && ktrhsUserId);
