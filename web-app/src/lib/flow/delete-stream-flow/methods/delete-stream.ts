...
import { getCallerClient, getNetworkConfig } from '$lib/utils/get-clients';
    ...
    makeTransactPayload({
      before: async () => {
        ...
        const newReceivers = currentReceivers.filter(
          (r) =>
            Utils.KtrhsReceiverConfiguration.fromUint256(r.config).ktrhId.toString() !==
            stream.ktrhsConfig.ktrhId,
        );
