  ...
  transact(
    ...
    makeTransactPayload({
      before: async () => {
        ...
        const ktrhId = randomBigintUntilUnique(
          assetConfig.streams.map((s) => BigInt(decodeStreamId(s.id).ktrhId)),
          4,
        );

        const ktrhConfig = Utils.KtrhsReceiverConfiguration.toUint256({
          ktrhId,
          start,
          duration,
        ...
        const newStreamMetadata: z.infer<typeof streamMetadataSchema> = {
          id: makeStreamId(ownUserId, tokenAddress, ktrhId.toString()),
          initialKtrhsConfig: {
            ktrhId: ktrhId.toString(),
            raw: ktrhConfig.toString(),
