...
function searchMatchStringsForStream(stream: Stream): MatchStrings {
  const { name } = stream;
  const { ktrhId } = stream.ktrhsConfig;
...
export default function search(input: string | undefined) {
  ...
  // Input is exclusively numeric, which means it may be a ktrhs user ID
  if (
    /^\d+$/.test(input) &&
    searchItems.findIndex(
      (i) => i.type === SearchItemType.PROFILE && i.item.ktrhsUserId === input,
    ) === -1
  ) {
    searchItems.push({
      type: SearchItemType.PROFILE,
