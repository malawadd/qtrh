<script lang="ts">
  import SearchIcon from 'design-system/icons/MagnifyingGlass.svelte';
  import CloseIcon from 'design-system/icons/CrossSmall.svelte';
  ...
  $: {
    const { ktrhsUserId } = $wallet;

    if (ktrhsUserId && $accountFetchStatussesStore[ktrhsUserId]?.all !== 'fetched') {
      loading = true;
    } else {
      loading = false;
    }
  ...
  $: {
    ...
    $tokens;
    if (!loading) {
      updateSearchItems($wallet.ktrhsUserId);
      results = search(searchTerm);
    }
  }
